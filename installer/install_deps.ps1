...
.DESCRIPTION
    Installs every runtime dependency needed by VoiceSyntesis:
      • Python 3.11        (runtime)
      • Node.js LTS        (runtime – reserved for future use / dev mode)
      • FFmpeg             (audio conversion)
      • Ollama             (local LLM service)
      • PyTorch            (CPU build, or CUDA if NVIDIA GPU detected)
      • Python packages    (FastAPI, faster-whisper, f5-tts, …)
    ...
    sources so it works on a completely fresh Windows PC with nothing pre-installed.
...
function Ok    { param($m) Log "  ✔  $m"        "Green"  }
function Warn  { param($m) Log "  ⚠  $m"        "Yellow" }
function Abort {
    param($m)
    Log "  ✖  $m" "Red"
    Write-Host ""
...
Write-Host "  ║  Do NOT close this window — it may take 20-40 minutes.           ║" -ForegroundColor Cyan
Write-Host "  ║                                                                  ║" -ForegroundColor Cyan
$modelsLine = if ($DownloadModels) { "  ║  AI models will be downloaded (~4.5 GB).                         ║" } `
                                  else { "  ║  AI models will download on first use (~4.5 GB).                 ║" }
Write-Host $modelsLine -ForegroundColor Cyan
Write-Host "  ╚══════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
# ══ STEP 2 — NODE.JS LTS ═════════════════════════════════════════════════════
Start-Step "Node.js LTS" "JavaScript runtime (reserved for dev mode)"
...
if ($nodeExe) {
    ...
    if (-not $nodeInstalled) {
        # The LTS MSI URL is stable — Node.js publishes it at /latest-v20.x/
    ...
    if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
        Warn "Node.js not detected after install. It may require a fresh terminal session."
    } else {
...
# Persist tools dir to user PATH
...
# ── Done ──────────────────────────────────────────────────────────────────────
$elapsedMin = [math]::Round(((Get-Date) - (Get-Item $logFile).CreationTime).TotalMinutes, 1)
Write-Host ""
Write-Host ("─" * 70) -ForegroundColor DarkCyan
Write-Host ""
Write-Host "  ╔══════════════════════════════════════════════════════════════════╗" -ForegroundColor Green
Write-Host "  ║   ✔  VoiceSyntesis installation complete!                        ║" -ForegroundColor Green
Write-Host "  ║                                                                  ║" -ForegroundColor Green
Write-Host "  ║   All $TOTAL_STEPS steps finished. You can now close this window.            ║" -ForegroundColor Green
Write-Host "  ║   Double-click the VoiceSyntesis shortcut on your desktop.       ║" -ForegroundColor Green
Write-Host "  ║                                                                  ║" -ForegroundColor Green
